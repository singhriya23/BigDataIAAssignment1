...
import requests
import os
from test_S3 import upload_to_s3  # Import S3 upload function
...
if dropdown == "Extract to Markdown (PyMuPDF)":
    ...
    if uploaded_file is not None:
        ...
        if response.status_code == 200:
            ...
            preview_length = min(1000, len(markdown_content))  # Show only first 1000 characters
...
elif dropdown == "Parse HTML (BeautifulSoup)":
    url = st.sidebar.text_input("Enter the URL of the PDF")
    if st.sidebar.button("Process") and url:
        response = requests.post(f"{BASE_API_URL}/parse-html/", data={"url": url})
        if response.status_code == 200:
            st.write("### Extracted Content")
            content = response.text
            preview_length = min(1000, len(content))  # Show only first 1000 characters
...
elif dropdown == "Process Doc Intelligence":
...
elif dropdown == "Extract Text with PyTesseract":
    uploaded_file = st.sidebar.file_uploader("Upload an Image (JPG, PNG, TIFF, PDF)", type=["jpg", "png", "tiff", "pdf"])
    if uploaded_file is not None:
        files = {"file": uploaded_file.getvalue()}  # Prepare file for API request
