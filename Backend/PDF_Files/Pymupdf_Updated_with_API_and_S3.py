...
async def extract_pdf(file: UploadFile = File(...)):
    ...
    # **Fixing the filename issue**
    original_filename = file.filename  # ✅ Correctly capture the uploaded filename
    pdf_name, file_extension = os.path.splitext(original_filename)
    
    # ✅ Ensure correct S3 folder naming using original filename
    pdf_s3_folder = f"{S3_PDF_OBJECT}/{pdf_name}/"
    temp_pdf_path = f"/tmp/{original_filename}"  # ✅ Use original filename instead of 'file'
    ...
    # ✅ Process and extract text, images, tables
